...
// -------------------------------------------------------------------------- //
// MARK: Double3x2Storage - Definition
// -------------------------------------------------------------------------- //

public struct Double3x2Storage {
  
  public typealias Storage = simd_double3x2
...
// -------------------------------------------------------------------------- //
// MARK: Double3x2Storage - NativeSIMDRepresentable
// -------------------------------------------------------------------------- //

extension Double3x2Storage : NativeSIMDRepresentable {
...
// -------------------------------------------------------------------------- //
// MARK: Double3x2Storage - SIMDMatrix3x2StorageProtocol
// -------------------------------------------------------------------------- //

extension Double3x2Storage : SIMDMatrix3x2StorageProtocol {
  ...
  public typealias TransposeStorage = Double2x3Storage
  ...
  public init(columns: [ColumnVector]) {
    precondition(columns.count == Double3x2Storage.columnCount)
    self.storage = Storage(
      columns[0],
      columns[1],
      columns[2]
  ...
  public init(rows: [RowVector]) {
    precondition(rows.count == Double3x2Storage.rowCount)
    self.storage = Storage(
      rows: [
        rows[0],
        rows[1]
  ...
  public init(arrayLiteral elements: Scalar...) {
    guard elements.count == type(of: self).scalarCount else {
      fatalError("Invalid array-literal construction: \(String(reflecting: elements)) supplied-to `Double3x2Storage`!")
    }
  ...
  public var description: String {
    get {
      return "Double3x2Storage(storage: \(String(describing: self.storage)))"
    }
  }
  ...
  public var debugDescription: String {
    get {
      return "Double3x2Storage(storage: \(String(reflecting: self.storage)))"
    }
  }
  ...
  public subscript(columnIndex columnIndex: Int) -> ColumnVector {
    get {
      precondition(Double3x2Storage.columnIndexRange.contains(columnIndex))
      switch columnIndex {
      case 0:
  ...
  public subscript(columnIndex columnIndex: Int, rowIndex rowIndex: Int) -> Scalar {
    get {
      precondition(Double3x2Storage.columnIndexRange.contains(columnIndex))
      precondition(Double3x2Storage.rowIndexRange.contains(rowIndex))
      switch columnIndex {
      case 0:
  ...
  @inlinable
  public func negated() -> Double3x2Storage {
    return Double3x2Storage(
  ...
  @inlinable
  public func adding(_ other: Double3x2Storage) -> Double3x2Storage {
    return Double3x2Storage(
  ...
  @inlinable
  public mutating func formAddition(of other: Double3x2Storage) {
    self.storage += other.storage
  }
  
  @inlinable
  public func adding(
    _ other: Double3x2Storage,
    multipliedBy factor: Scalar) -> Double3x2Storage {
    return Double3x2Storage(
  ...
  public mutating func formAddition(
    of other: Double3x2Storage,
    multipliedBy factor: Scalar) {
    self.storage += (other.storage * factor)
  }
  
  @inlinable
  public func subtracting(_ other: Double3x2Storage) -> Double3x2Storage {
    return Double3x2Storage(
  ...
  @inlinable
  public mutating func formSubtraction(of other: Double3x2Storage) {
    self.storage -= other.storage
  }
  
  @inlinable
  public func multiplied(by factor: Scalar) -> Double3x2Storage {
    return Double3x2Storage(
